...
class Mission:
    ...
    def starter(self, language, difficulty):
        ...
        lines[first_action.line - 1] = lines[first_action.line - 1].replace(first_action.value + '()', '???')
        return '\n'.join(lines) + '\n'
...
MISSIONS = (
    Mission('ricarica', '01 · Una ricarica se serve', 'Un if può anche non fare nulla.', 'Prime scelte', 'if',
        'Se la batteria è sotto il 30%, esegui ricarica(). Negli altri casi non eseguire azioni.',
    ...
    Mission('soglia', '03 · Pronti al decollo', 'Il confine è un caso da provare.', 'Prime scelte', 'else',
        'Con almeno il 30% di batteria esegui parti(). Con meno del 30% esegui ricarica().',
    ...
    Mission('annidato', '10 · Dentro il modulo', 'Il controllo interno ha una porta d’ingresso.', 'Annidamento', 'nested',
        'Senza badge esegui nega(). Con badge controlla il livello: se è almeno 2 apri(), altrimenti accompagna(). Usa un if dentro il ramo vero del primo.',
